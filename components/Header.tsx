...
export const Header: React.FC<HeaderProps> = ({ onSelectKey, apiKeyReady }) => {
  return (
    <header className="w-full py-5 px-5 md:py-8 md:px-12 flex items-center justify-between border-b border-slate-800/40 bg-slate-950/90 backdrop-blur-3xl sticky top-0 z-[100] pointer-events-auto">
      ...
      <div className="flex items-center gap-4">
        <button 
          onPointerDown={(e) => { e.preventDefault(); onSelectKey(); }}
          type="button"
          className={`relative flex items-center justify-center gap-3 px-6 py-4 md:px-6 md:py-3 rounded-2xl md:rounded-[20px] text-[10px] md:text-xs font-black uppercase tracking-widest transition-all border active:scale-95 touch-manipulation cursor-pointer min-h-[48px] min-w-[120px] ${
            apiKeyReady 
